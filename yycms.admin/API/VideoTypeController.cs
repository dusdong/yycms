...
namespace yycms.admin.API
{
    ...
    /// </summary>
    [BasicAuthen]
    public class VideoTypeController : BasicAPI
    {
        #region  视频分类列表
        /// <summary>
        /// 获取视频分类列表
        ...
        [HttpPost]
        public async Task<ResponseData<yy_Video_Type>> Get(RequestEntity value)
        {
            //查询的表名称
            Type Table = typeof(yy_Video_Type);
            ...
            return new ResponseData<yy_Video_Type>(value.PageSize,
                value.PageIndex,
                DataCount,
                (DataCount % value.PageSize == 0 ? DataCount / value.PageSize : DataCount / value.PageSize + 1),
                DB.Database.SqlQuery<yy_Video_Type>(QuertCMD).ToList());
            #endregion
        }
        #endregion
        ...
        /// <returns></returns>
        [HttpGet]
        public yy_Video_Type Get(int id)
        {
            return DB.yy_Video_Type.Find(id);
        }
        #endregion
        ...
        [HttpPost]
        public ResponseItem Post(yy_Video_Type value)
        {
            try
            {
                DB.yy_Video_Type.Add(value);
                DB.SaveChanges();
                return new ResponseItem(0, "添加视频分类成功。");
            }
            catch (Exception ex)
            {
        ...
        /// <param name="value">视频分类实体。</param>
        [HttpPut]
        public ResponseItem Put(yy_Video_Type value)
        {
            var _Entity = DB.yy_Video_Type.Find(value.ID);
            ...
            return new ResponseItem(2, "不存在的视频分类。");
        }
        #endregion
        ...
        {
            DB.Database.ExecuteSqlCommand("DELETE yy_Video_Type WHERE ID = " + id);
            return new ResponseItem(0, "");
        }
        #endregion
        ...
        /// <returns></returns>
        [HttpPut]
        public ResponseItem ShowHide(yy_Video_Type value)
        {
            var _News = DB.yy_Video_Type.Find(value.ID);
            if (_News != null)
            {
        ...
        public ResponseItem ShowByIDs(String ids)
        {
            ...
            foreach (var v in IDs)
            {
                ...
                if (long.TryParse(v, out id))
                {
                    DB.Database.ExecuteSqlCommand("UPDATE yy_Video_Type SET IsShow = 1 WHERE ID = " + id);
                }
            }
            return new ResponseItem(0, "");
        }
        ...
        public ResponseItem HideByIDs(String ids)
        {
            ...
            foreach (var v in IDs)
            {
                ...
                if (long.TryParse(v, out id))
                {
                    DB.Database.ExecuteSqlCommand("UPDATE yy_Video_Type SET IsShow = 0 WHERE ID = " + id);
                }
            }
            return new ResponseItem(0, "");
        }
